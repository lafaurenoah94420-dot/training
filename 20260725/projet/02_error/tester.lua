...
if type(main.alerter) ~= "function" then
  print("❌ Erreur : la fonction alerter n'est pas définie dans main.lua")
  print("   → Vérifie que tu as bien écrit : function alerter(...)")
  os.exit(1)
end

local ok, err = pcall(main.alerter)
_tester("alerter() échoue (ok == false)", ok, false)
_tester('le message contient "infection"', string.find(tostring(err), "infection", 1, true) ~= nil, true)
